...
$lastpoly = `tail -1 $ARGV[0]`;

chomp $lastpoly;
($junk,$junk,$oldutc,$oldpoly) = split(' ',$lastpoly,4);

# open the history file, to append to it
open (HF, ">>$ARGV[0]") or die "$0: Cannot open polygon history file for appending: $ARGV[0]\n";
...
$polydata = "MnemRet.py -e '+20 minutes' -b '$date' --expr 'VSGSAA1PLONG[01][0-9]' --expr 'VSGSAA1PLAT[01][0-9]' --csv /dev/stdout";
...
# read the new data one line at a time
# report the time difference to STDERR
# if the new polygon does not match the old, then add the new polygon to the history.new file
