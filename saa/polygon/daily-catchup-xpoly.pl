...
# usage: ./daily-catchup-xpoly.pl < w2 >> daily.history.poly

$cmd1 = "MnemRet.py -e '+20 minutes' -b '";
$cmd2 = "' --expr 'VSGSAA[12]PLONG[01][0-9]' --expr 'VSGSAA[12]PLAT[01][0-9]' --csv /dev/stdout | xpoly.pl";

# run the constructed command for each day, but only output new instances of the polygon

$| = 1;
$prev = "";
foreach $d (1..500) {
    $date = `date -u --date="2016-03-20 + $d days" +"%F %T"`;
    chomp $date;
    $cmd = $cmd1.$date.$cmd2;
#    print "$cmd\n";
    @out = `$cmd`;
