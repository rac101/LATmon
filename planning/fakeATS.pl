...
# make a routine weekly LAT ATS Review or Planned listing as shown in MP web viewer
...
# 2016 November: merge fakeATSreview and fakePlanned into this code
# 2017 February: add warnings for LAT commands within a few seconds of mission week boundary
...
$ofsuccess = 0;
if ($ofile) { 
    $product = $plan? "plan" : "rev";
    $outfile = "$mproot/$mpmw$product.fake";
    exit if (-e $outfile and !$force);
...
$postsaa = 2730; # minimum seconds after SAA exit when an Ascending Node cannot have LPA stop and start
...
$SAAwindow = 60;  # seconds of concern between an SAA entry/exit and the start or end of the planned MW
$ANwindow = 15;  # seconds of concern between an AN crossing and the start or end of the planned MW
...
# check time against MW boundary times
    print STDERR "$sn: WARNING! SAA entry within $SAAwindow seconds of the start of MW $mpmw\n" if (abs($time - $s_mw1) < $SAAwindow);
    print STDERR "$sn: WARNING! SAA entry within $SAAwindow seconds of the end of MW $mpmw\n" if (abs($time - $s_mw2) < $SAAwindow);
...
# check time against MW boundary times
    print STDERR "$sn: WARNING! SAA exit within $SAAwindow seconds of the start of MW $mpmw\n" if (abs($time - $s_mw1) < $SAAwindow);
    print STDERR "$sn: WARNING! SAA exit within $SAAwindow seconds of the end of MW $mpmw\n" if (abs($time - $s_mw2) < $SAAwindow);
    ($day, $year) = split('/',$field[0]);
    $daynum = $daycount{$year} + $day;
    ($hour, $minute, $second) = split(':',$field[1]);
    $time = $daynum*86400 + $hour*3600 + $minute*60 + $second;
...
# and not earlier than $POSTSAA seconds after an SAA exit
...
# check time against MW boundary times
    print STDERR "$sn: WARNING! Ascending Node within $ANwindow seconds of the start of MW $mpmw\n" if (abs($time - $s_mw1) < $ANwindow);
    print STDERR "$sn: WARNING! Ascending Node within $ANwindow seconds of the end of MW $mpmw\n" if (abs($time - $s_mw2) < $ANwindow);
...
$head4 = $plan? "Type,Priority,State,Status,Description" : "CMD Time,Priority,Description,Command,Arguments";
print OF "$head1 $head2\n";
print OF "#Data products used: $ephemfilename $saafilename\nStart Time,End Time,$head4\n";

# print ATS content
$planbase = "Acquisition,NORMAL,InReview,Unknown,[$mpmw] Normal acquisition (default configuration)";
$revbase = "NORMAL,[$mpmw] Normal acquisition (default configuration),";
$revbeg = $revbase.'LPASTART,"LPACPUS=0x6,   LPADBID=0x0,   LPALATCCFGID=0xffffffff,   LPALATCCNSFLG=0x1,   LPALATCIGNID=0xffffffff,   LPAMODEID=0x0,   LPARUNID=0"';
$revend = $revbase."LPASTOP,";
$revbias = $revbase."LATBIASVUP,";
...
foreach $tt (@times) {
    ...
    $run = "";
    if ($plan) { 
	$run = "$tbeg,$tend,$planbase\n";
    } else {
    $tcmd1 = ($eventx =~ /Sx/)? formtime($tx + $saapadtime) : formtime($tx);
    $tcmd2 = ($event =~ /Se/)? formtime($tt - $saapadtime) : formtime($tt);
    $run .= "$tbeg,$tend,$tbeg,$revbias\n" if ($eventx =~ /Sx/);
    $run .= "$tbeg,$tend,$tcmd1,$revbeg\n";
    $run .= "$tbeg,$tend,$tcmd2,$revend\n";
    }
...
if ($plan) {
# define output line for weekly LATSUROMDUMP RTS call at 00:00:00 Friday
    $time = $day2*86400;
#    while ($lines{$time}) { $time++ };
    $t0 = formtime($time);
    $lines{$time} = "$t0,$t0,ATSList,NORMAL,InReview,Unknown,[$mpmw] LATSUROMDUMP Activity (MPLAN-679)\n";
}

@times = sort { $a <=> $b } keys %lines;
foreach (@times) { print OF $lines{$_} };
