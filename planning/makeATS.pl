...
GetOptions ("force" => \$force, # flag: 1 = run to completion even if dates are wrong or outfile exists
            ...
            "track" => \$track) # flag: 1 = update database with runID and tranID bounds from this MW ATS
    or die("$sn: Error in command line arguments\n");

# get optional command line switches and command line ARGS: starting runID and trandID values
$runid = 0;
$tranid = 0;
$runid0 = 0;
$tranid0 = 0;
...
# if re-planning a MW already in the makeATS database file, use the same starting ID values
    ...
    print STDERR "$sn: Warning: discontinuity in planned MWs, from $lastweek to $mpmw\n" unless (($mpmw - $lastweek) == 1);
    print STDERR "$sn: NOTE: re-planning current MW $mpmw\n" if ($thisweek);
...
foreach (@times) { 
    $runid{$_} = 0;
    $runid{$_} = $runid++ if (!$zeroid and ($_ >= $s_mw1) and ($_ <= $s_mw2));
}
...
# define ATS line for weekly LATSUROMDUMP RTS at 00:00:00 Friday
$time = $day2*86400;
while ($lines{$time}) { $time++ };
$t0 = formtime($time);
$lines{$time} = "$t0\tACT\tLATSUROMDUMP;\n";

# the following commands are obsolete, replaced by the LATSUROMDUMP RTS
...
if ($track) { 
    $ptr = 0;
    $ptr++ until ($runid{$times[$ptr]} > 0);
    $runid0 = $runid{$times[$ptr]};
    $ptr = -1;
    $ptr-- until ($runid{$times[$ptr]} > 0);
    $runid1 = $runid{$times[$ptr]};
    $tranid0 = $tranid-2;
