...
# output goes to "idl-YYYY-qQ.inp", which is then transferred to the Mac for plotting in IDL

#Usage: > ./getqdat.pl
##e.g.   > ./getqdat.pl produces "idl-2012-q4.inp"
...
$month = `date +%m`;
chomp $month;
@quarter = (4, 4, 4, 4, 1, 1, 1, 2, 2, 2, 3, 3, 3);
$qtr = $quarter[$month];
$year-- if ($qtr == 4);
$outfile = "idl-$year-q$qtr.inp";
...
print STDERR "$0: output goes to file: $outfile\n";
open (AF,'geosaa.out') or die "$0: Cannot open archive data file\n";
open (OF,">$outfile") or die "$0: Cannot open output file: $outfile\n";
$ctot = 0;
$cin = 0;
$fin = 0;
while (<AF>) {
    ...
    $ctot++;
    $cin++ if ($f[14]);
    print OF "$y $month $d $h $m $s $f[3] $f[12] $f[13] $f[14]\n";
}
$fin = sprintf "%4.1f", $cin*100/$ctot;
print STDERR "$0: $ctot Total Errors\n";
print STDERR "$0: $fin% inside SAA\n";
