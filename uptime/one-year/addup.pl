...
print $jdnow-$jd2008, " days since 2008 August 4\n\n";
...
printf "Day Count = %d (NOTE: %d days*86400 = %d seconds)\n",$day_ctr,$day_ctr,$day_ctr*86400;
printf "Total time = %d\n",$tot;
...
printf "LCI effective fraction (LCI run+stop time/Total-SAA time) = %8.5f\n",($lci_run+$lci_stop)/($tot-$saa);
